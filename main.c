...
void controleBomba(BOMBA *bomba, char matriz[][COLUNA], int tecla, JOGADOR jogador, CRIATURA criatura[], int qtdCriatura)
{
    ...
    for(i = 0; i < QTD_BOMBA; i++){
        if(tempoCmp(bomba[i].tempo, 4) && bomba[i].estado == SOLTA){ // Se acabar o tempo a bomba explode
            ...
            bomba[i].tempoExp = GetTime();


        }
        else if(tempoCmp(bomba[i].tempoExp, .15) && bomba[i].estado == EXPLODIU){ // Finalizando a explosao da bomba
            bomba[i].estado = LIVRE;
        }
    }
}
...
void interacoes(JOGADOR *jogador, BOMBA *bomba, CRIATURA *criatura, char mapa[][COLUNA], int qtdCriatura)
{
    ...
    if(bomba->estado == EXPLODIU)// verificacao de colisao da explosao
        for(i = 0; i < EXPLOSAO * EXPLOSAO; i++){
            if(!verifColisMap(bomba->xExp[i], bomba->yExp[i], mapa, "D"))
                mapa[bomba->yExp[i]][bomba->xExp[i]] = ' ';
            if(!verifColisCord(bomba->xExp[i], bomba->yExp[i], jogador->xPos, jogador->yPos)){ // Se a bomba explodir no jogador
                printf("\nSofri dano da bomba");
            }
            for(j = 0; j < qtdCriatura; j++){
                if(criatura[i].tipo == 'M' && !verifColisCord(bomba->xExp[i], bomba->yExp[i], criatura[j].xPos, criatura[j].yPos)) // Se a posicao do monstro for a mesma da bomba
                    printf("\nMonstro morreu");
            }
