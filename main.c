...
int main(void)
{
    ...
    srand(time(NULL));
    cont = leMapa( jogo.arquivo, matriz, &jogador, criatura, &jogo.xTam, &jogo.yTam);
    ...
    do{
        ...
        if(jogo.estado == JOGANDO){
            ...
            while (!WindowShouldClose() && jogo.estado == JOGANDO){// Detecta se deve continuar e faz o jogo acontecer
                ...
                if(!verifFimJogo(matriz)){
                    ...
                    leMapa(jogo.arquivo,matriz,&jogador,criatura, &jogo.xTam, &jogo.yTam);
                }
        ...
        if(jogo.estado == MENU){
            ...
            while(jogo.estado == MENU){
                ...
                case KEY_N:
                    jogo.estado = JOGANDO;
                    cont = leMapa("mapa1.txt",matriz,&jogador,criatura, &jogo.xTam, &jogo.yTam);
                    initJogo(&jogador, criatura, bomba, &jogo, cont);
                    break;
                case KEY_V:
                    jogo.estado = JOGANDO;
                ...
                case KEY_Q:
                    ...
                    break;
                case KEY_C:
                    leMapa("save.txt", matriz, &jogador, criatura, &jogo.xTam, &jogo.yTam);
...
void desenhaMapa(char matriz[][N],  JOGADOR jogador, CRIATURA criatura[], BOMBA bomba[], JOGO jogo, int cont){
    ...
    ClearBackground(BLACK);//Limpa e define uma cor de fundo
        ...
        DrawText( texto, BORDA/2, 40 * LINHA + BORDA/2, BORDA/2, RAYWHITE);
        ...
        if(criatura[i].vida){
            if(criatura[i].movCont == 5 || !podeMover(criatura[i].xPos, criatura[i].yPos, mapa, criatura[i].direcao, LINHA, COLUNA, bomba)){ // Se tiver se movido 5 vezes ou bater numa parede
                ...
                do{
                    ...
                    qtdTeste++;
                }while(!podeMover(criatura[i].xPos, criatura[i].yPos, mapa, criatura[i].direcao, LINHA, COLUNA, bomba) && qtdTeste < 10); // Garantindo que a nova direcao seja uma direcao valida
            }
...
void desEstoqueBomba(BOMBA bomba[], JOGADOR jogador)
{
    ...
    for(i = 0; i < QTD_BOMBA; i++)
        if(bomba[i].estado == LIVRE)
            DrawCircle( basX + i * varX , basY + i*varY, 8 , WHITE);
}
