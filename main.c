...
int main(void)
{
    ...
    while (!WindowShouldClose())// Detecta se deve continuar
    {
        ...
        else if(tempoCmp(bomba.tempo, 4))
        {
            bomba.plantada = 0;
        }
...
{
    bomba -> tempo = GetTime(); // Vendo o instante que a bomba foi plantada
    bomba -> plantada = 1;
    movDir(&jogador.xPos, &jogador.yPos, jogador.ultMov);
...
int podeMover( int xPos, int yPos, char matriz[][COLUNA], int tecla, int linha, int coluna, BOMBA bomba)
{
    ...
    if(xPos >= 0 && xPos < coluna && yPos >= 0 && yPos < linha && matriz[yPos][xPos] != 'W' && matriz[yPos][xPos] != 'D'
       && (xPos != bomba.xPos || yPos != bomba.yPos || !bomba.plantada ))
        flag_mov = 1;
